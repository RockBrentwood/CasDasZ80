...
// See below for details and example implementation of `ihex_data_read`.
//
...
// The `ihex` structure will have its fields `data`, `line_length`, `address`, and `segment` set appropriately.
// In case of reading an `IHEX_EXTENDED_LINEAR_ADDRESS_RECORD` or an `IHEX_EXTENDED_SEGMENT_ADDRESS_RECORD` the record's data is not yet parsed -
// it will be parsed into the `address` or `segment` field only if `ihex_data_read` returns `true`.
// This allows manual handling of extended addresses by parsing the `ihex->data` bytes.
//
// Possible error cases include checksum mismatch (which is indicated as an argument), and excessive line length
// (in case this has been compiled with `IHEX_LINE_MAX_LENGTH` less than 255) which is indicated by `line_length` greater than `length`.
