...
// Usage
// ─────
// The library has been split into read and write parts, which use a common data structure (`struct ihex_state`), but each can be used independently.
...
// The bytes read must then be passed to `ihex_read_byte` and/or `ihex_read_bytes`.
// The reading functions will then call `ihex_data_read`, at which stage the `struct ihex_state` structure will contain the data along with its address.
// See the header `HexIn.h` for details and example implementation of `ihex_data_read`.
//
...
// Conserving memory
// ─────────────────
// For memory-critical use, you can save additional memory by defining `IHEX_LINE_MAX_LENGTH` as something less than 255.
...
// you can provide your own buffer for the duration by defining `IHEX_EXTERNAL_WRITE_BUFFER` and providing a `char *ihex_write_buffer`
// which points to valid storage for at least `IHEX_WRITE_BUFFER_LENGTH` characters from before the first call to any IHEX write function to until after the last.
//
// If you are doing both reading and writing, you can define the maximum output length separately as `IHEX_MAX_OUTPUT_LINE_LENGTH` -
// this will decrease the write buffer size, but `struct ihex_state` will still use the larger `IHEX_LINE_MAX_LENGTH` for its data storage.
//
// You can also save a few additional bytes by disabling support for segmented addresses, by defining `IHEX_DISABLE_SEGMENTS`.
...
// Maximum number of data bytes per line (applies to both reading and writing!); specify 255 to support reading all possible lengths.
...
//
// To implement fully correct segmented addressing, compute the address of _each byte_ with its index in `data` as follows:
